...
export class PoseService {
  ...
  static async loadPoseConfig(preset, animationOptions = {}) {
    ...
    if (
      animationOptions.updateParamsOnly &&
      this.currentAnimation &&
      this.currentPreset
    ) {
      if (animationOptions.fps !== this.currentFps) {
        console.log(
          "FPS update requested from",
          this.currentFps,
          "to",
    ...
    if (animationOptions.isAnimation) {
      ...
      if (isMultiFrameAnimation) {
        ...
        console.log(
          "Starting multi-frame animation with FPS:",
          this.currentFps
        );
        ...
        // For pose animation with just one frame
        else if (poseData.pose && Array.isArray(poseData.pose)) {
          await this.sendSMPLPose(
            poseData.pose[0],
            poseData.trans?.[0],
            poseData.model,
        ...
        // For qpos
          ...
          const qpos = Array.isArray(poseData.pose.qpos)
            ? poseData.pose.qpos[0]
            : poseData.pose.qpos;
          await this.sendQPosPose(qpos, poseData.pose.inference_type);
        }
      }
    } else {
      ...
      if (preset.type === "pose" || preset.data?.pose) {
        ...
        if (poseData.pose?.pose && Array.isArray(poseData.pose.pose)) {
          console.log("Legacy path 2");
          await this.sendSMPLPose(
            poseData.pose.pose[0],
            poseData.pose.trans?.[0],
            poseData.pose.model,
  ...
  static isMultiFrameAnimation(preset) {
    ...
    if (
      preset.data.pose?.pose &&
      Array.isArray(preset.data.pose.pose) &&
      preset.data.pose.pose.length > 1
    ) {
      return true;
    }

    if (
      preset.data.pose?.qpos &&
      Array.isArray(preset.data.pose.qpos) &&
      preset.data.pose.qpos.length > 1
    ) {
      return true;
    }
  ...
  static startFpsLogging() {
    ...
    this.fpsLoggingInterval = setInterval(() => {
      ...
      console.log(
        `FPS Stats: Target=${this.currentFps}, Actual=${actualFps.toFixed(
          2
        )}, Frames sent=${this.frameCounter}`
      );
  ...
  static updateAnimationFps(newFps) {
    ...
    if (Array.isArray(this.currentPreset.data.pose)) {
      frameIndices = this.getFrameIndices(
        this.currentPreset.data.pose.length,
        newFps
      );
      console.log(
    ...
    const playFrame = async () => {
      ...
      if (processingTime > frameDelay * 0.8) {
        console.warn(
          `Frame processing time (${processingTime}ms) is close to or exceeds frame delay (${frameDelay}ms)`
        );
      }
  ...
  static async handleAnimationPlayback(
    preset,
    fps = this.CONFIG.DEFAULT_FPS,
    startFrame = 0
  ) {
    console.log(
      "Playing animation:",
    ...
    // Handle the nested array format for animations
      ...
      console.log(
        "Total frames in animation (nested format):",
        preset.data.pose.pose.length
      );
      console.log("Frames to display at current FPS:", frameIndices.length);
