...
class WebRTCService {
  ...
  async initWebRTC() {
    ...
    if (this.connectionAttempts >= CONNECTION_CONFIG.maxAttempts) {
      this.logger.log(
        `Maximum reconnection attempts (${CONNECTION_CONFIG.maxAttempts}) reached`
      );
      isLoading.set(false);
      return;
    }
  ...
  setupPeerConnectionEvents() {
    ...
    this.peerConnection.oniceconnectionstatechange = () => {
      ...
      if (state === "connected" || state === "completed") {
        ...
        this.logger.log(
          "ICE connection disconnected, waiting to see if it recovers"
        );
        setTimeout(() => {
          if (
            this.peerConnection &&
            ...
            this.logger.log(
              "ICE connection still disconnected, handling as failure"
            );
            hasStream.set(false);
            this.handleConnectionFailure();
          }
  ...
  handleConnectionFailure() {
    ...
    if (this.connectionAttempts >= CONNECTION_CONFIG.maxAttempts) {
      this.logger.log(
        `Max reconnection attempts (${CONNECTION_CONFIG.maxAttempts}) reached`
      );
      isLoading.set(false);
      ...
      this.reconnectTimeout = setTimeout(() => {
        this.logger.log(
          "Resetting connection retry counter and attempting reconnection"
        );
        this.connectionAttempts = 0;
        if (websocketService.isConnected()) {
          this.initWebRTC();
